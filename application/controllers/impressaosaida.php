...
class impressaosaida extends CI_Controller {
    ...
    public function imprimir_saida_finalizadas()
    {
	   ...
	   $saida = $this->db->get('saida_semem')->result();
        ...
        $pdf = new PDF();
        $pdf->AddPage('L','A4',0);
        $pdf->setTitle(utf8_decode("LISTA DE SAÍDAS"));
		$pdf->SetFont('Arial','B',18);
		$pdf->Cell(270,5,utf8_decode("LISTA DE SAÍDAS FINALIZADAS") ,0,0,"C");
		$pdf->Ln(10);	
		$pdf->SetFont('Arial','B',10);
		$pdf->Cell(10,5,"ID" ,0,0);	
		$pdf->Cell(80,5,"NOME" ,0,0);
		$pdf->Cell(25,5,utf8_decode("BOTIJÃO") ,0,0);
		$pdf->Cell(25,5,"ANIMAL" ,0,0);
		$pdf->Cell(20,5,utf8_decode("CÓDIGO") ,0,0);
		$pdf->Cell(15,5,"QTDE" ,0,0);
		$pdf->Cell(30,5,"EST.ANTERIOR" ,0,0);
		$pdf->Cell(40,5,"TIPO" ,0,0);
		$pdf->Cell(25,5,utf8_decode("CONCLUSÄO") ,0,1);
        ...
        if(sizeof($saida)>0){
            foreach($saida as $row) {
              ...
              $aplicador=$row->idsaidaaplicador;
              $d=$row->datasaida;
              $data=formatardata1($d);

			$pdf->Cell(10,5,utf8_decode($row->idsaida) ,1,0);	
			$pdf->Cell(80,5,utf8_decode($row->nome) ,1,0);	
			$pdf->Cell(25,5,utf8_decode($row->nomebotijao) ,1,0);
			$pdf->Cell(25,5,utf8_decode($row->nomeanimal) ,1,0);
			$pdf->Cell(20,5,utf8_decode($row->estcodigo) ,1,0);
			$pdf->Cell(15,5,utf8_decode($row->qtde) ,1,0);
            $pdf->Cell(30,5,utf8_decode($row->estoqueanterior) ,1,0);
			$pdf->Cell(40,5,utf8_decode($tipoarray[$tipo]) ,1,0);
			$pdf->Cell(25,5,$data ,1,1);
			$i++;	
        }
        }
        
		$pdf->Cell(270,5,"Total de cadastros: ".$i,1,1);
    ...
    public function imprimir_saida_afinalizar()
    {
	   ...
	   $saida = $this->db->get('saida_semem')->result();
        ...
        $pdf->setTitle(utf8_decode("LISTA DE SAÍDAS"));
		$pdf->SetFont('Arial','B',18);
		$pdf->Cell(270,5,utf8_decode("LISTA DE SAÍDAS A FINALIZAR") ,0,0,"C");
		$pdf->Ln(10);	
		$pdf->SetFont('Arial','B',10);
		$pdf->Cell(10,5,"ID" ,0,0);	
		$pdf->Cell(80,5,"NOME" ,0,0);
